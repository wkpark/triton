...
bool isMfmaToDotShortcut(RankedTensorType srcTy, RankedTensorType dstTy) {
  auto mfmaLayout =
      dyn_cast<triton::gpu::AMDMfmaEncodingAttr>(srcTy.getEncoding());
  auto dotOperandLayout =
      dyn_cast<triton::gpu::DotOperandEncodingAttr>(dstTy.getEncoding());
...
bool cvtReordersRegisters(RankedTensorType srcTy, RankedTensorType dstTy) {
  ...
  if (srcLayout.has_value() && dstLayout.has_value()) {
    ...
    if (comp.divideRight(triton::LinearLayout::identity1D(
                             comp.getInDimSize(kLane), kLane, kLane) *
                         triton::LinearLayout::identity1D(
                             comp.getInDimSize(kWarp), kWarp, kWarp) *
                         triton::LinearLayout::identity1D(
                             comp.getInDimSize(kBlock), kBlock, kBlock))
            .has_value()) {
      return true;
    }
  }
